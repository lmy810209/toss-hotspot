  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* 제보 TOP 5 + source/sourceCategory 통계 */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              ...
              <div className="flex flex-col gap-1">
                {[...spots].sort((a, b) => b.report_count - a.report_count).slice(0, 5).map((s) => (
                  <span key={s.id} className="text-[11px] bg-blue-50 text-blue-700 font-bold px-2 py-1 rounded-lg">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            <p className="text-xs font-black text-gray-700 mb-2">📦 source별</p>
            <div className="flex flex-col gap-1">
              {Object.entries(
                spots.reduce((acc, s) => { acc[s.source || "unknown"] = (acc[s.source || "unknown"] || 0) + 1; return acc; }, {} as Record<string, number>)
              ).sort((a, b) => b[1] - a[1]).map(([src, cnt]) => (
                <div key={src} className="flex justify-between text-[11px]">
                  <span className="text-gray-600">{src}</span>
                  <span className="font-bold text-gray-900">{cnt}개</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            <p className="text-xs font-black text-gray-700 mb-2">🏷️ 네이버 카테고리별 (상위 10)</p>
            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto">
              {Object.entries(
                spots.reduce((acc, s) => {
                  const cat = s.sourceCategory || "(없음)";
                  acc[cat] = (acc[cat] || 0) + 1;
                  return acc;
                }, {} as Record<string, number>)
              ).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([cat, cnt]) => (
                <div key={cat} className="flex justify-between text-[11px]">
                  <span className="text-gray-600 truncate mr-2">{cat}</span>
                  <span className="font-bold text-gray-900 shrink-0">{cnt}개</span>
                </div>
              ))}
            </div>
          </div>
